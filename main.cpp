...
int main(int argc, char* argv[])
{
	...
	TestCases testInd =  TestCases::SoftNMS; //TestCases::OnlyRectangles2;
    ...
    NMS nms(srcRects);
    NMS2 nms2(srcRects, scores, 0.5f);
	...
	case TestCases::OnlyRectangles2:
		nms.calculateNMS(resRects, 0.3f, 1);
        std::cout<< "size -> "<< srcRects.size() << " rest ->" << resRects.size()<< std::endl;

		break;

	case TestCases::RectanglesScores:
	case TestCases::RectanglesScoresMinSum:
		nms2.calculateNMS2(resRects, 0.3f, 1, minScoresSum);
        std::cout<< "size -> "<< srcRects.size() << " rest ->" << resRects.size()<< std::endl;
		break;

	case TestCases::SoftNMS:
		nms2.calculateSoftNMS(resRects, resScores, 0.3f, Methods::GaussNMS, 0.5f);
		break;
	}
